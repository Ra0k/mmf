...
class LoRRA2(Pythia):
    ...
    def forward(self, sample_list):     
        ...
        if visualization:
            np.save('visualization/question_id', sample_list.question_id.cpu().data.numpy())
            np.save('visualization/image_id', sample_list.image_id.cpu().data.numpy())
            np.save('visualization/image_bbox', sample_list.image_info_0.bbox)
            np.save('visualization/ocr_bbox', sample_list.ocr_bbox_coordinates.cpu().data.numpy())
        ...
        if visualization:
            np.save('visualization/image_attentions', np.array(image_attentions[0]))
            np.save('visualization/context_attentions', np.array(context_attentions[0]))
        ...
        if visualization:
            np.save('visualization/image_plus_attentions', np.array(image_plus_attention))
            np.save('visualization/context_plus_attentions', np.array(context_plus_attention))
        
        image_attentions[0] = softmax(image_attentions[0] + (image_attentions[0] * image_plus_attention), dim=1)
        context_attentions[0] = context_attentions[0] + (context_attentions[0] * context_plus_attention)
        
        if visualization:
            np.save('visualization/image_final_attentions', np.array(image_attentions[0]))
            np.save('visualization/context_final_attentions', np.array(context_attentions[0]))
